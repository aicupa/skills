...
description: Todolist desktop app knowledge — .todo file format, plugin system, plugin API, view protocol, communication architecture patterns, plugin contributes, board management workflow, and webhook sync. Use this skill whenever working with .todo files, building or debugging Todolist plugins, implementing plugin views or head views, choosing a plugin architecture (view+service, inject-only, view+inject, head-view-bridge, plugin-call-service, etc.), implementing plugin communication (postMessage RPC, localStorage bridge, inject.js patterns, tree-update push model, hidden head view bridge, plugin-call-service CustomEvent, hover-based lazy decorations), working with the Todolist plugin API and contributes system (context menus, events, views), maintaining a planning board (calendar scheduling via date ranges, monthly milestone tags with timeline true, focus view, weekly rolling files, simpleMode default for read-only boards, anti-pileup month splitting, multi-writer human+agent sync with content-based dedup, done-as-archive, tree hygiene, desc summary slot), or setting up webhook sync from the local app to a self-hosted web board (Settings webhook URL, board /api endpoint, uid/file params, Store/saveConfig event effects, expandKeys vs explorer expandedKeys), or an agent/script directly reading and writing the web board over its /api (GetStore read, Store full-snapshot write, eventId mirrors version, currentKey auto-switch on new files). Also use when users mention todolist format, todo tree structure, plugin service methods, plugin iframe communication, inject.js, callPlugin, double-unwrap, plugin architecture selection, inject.js service access, head view bridge, plugin-call-service, context menu node custom fields, plugin file sync, webhook sync, sync todo to web board, calendar/schedule a todo, milestone tags, focus view, weekly todo rollover, or expandKeys/explorer state sync.
---
