...
description: Todolist desktop app knowledge — .todo file format, plugin system, plugin API, view protocol, communication architecture patterns, plugin contributes, and webhook sync. Use this skill whenever working with .todo files, building or debugging Todolist plugins, implementing plugin views or head views, choosing a plugin architecture (view+service, inject-only, view+inject, head-view-bridge, plugin-call-service, etc.), implementing plugin communication (postMessage RPC, localStorage bridge, inject.js patterns, tree-update push model, hidden head view bridge, plugin-call-service CustomEvent, hover-based lazy decorations), working with the Todolist plugin API and contributes system (context menus, events, views), or setting up webhook automation from the Settings webhook field (event catalog by user action, Store/pushBackupVersion/saveConfig params, expandKeys vs explorer expandedKeys, local receiver bridging .todo files). Also use when users mention todolist format, todo tree structure, plugin service methods, plugin iframe communication, inject.js, callPlugin, double-unwrap, plugin architecture selection, inject.js service access, head view bridge, plugin-call-service, context menu node custom fields, plugin file sync, webhook automation, sync todo to web, or expandKeys/explorer state sync.
---
...
| `references/webhook-sync.md` | Webhook automation — Settings→Webhook config/activation, `callService` hook event shape (`{service, params, eventId}`), event catalog by user action (`Store`+`pushBackupVersion` on save, `saveConfig` for explorer expandKeys vs `Store` value.expandKeys for tree nodes), zero-dependency local receiver mirroring `.todo` files, pitfalls |
